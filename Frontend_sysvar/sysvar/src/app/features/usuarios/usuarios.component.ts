...
export class UsuariosComponent implements OnInit {
  ...
  usernameValidator(control: AbstractControl): ValidationErrors | null {
    ...
    if (!v) return null;
    // sem espaços; aceitar letras, números, _ . -
  ...
  private normalizePayload(raw: any): Partial<User> {
  ...
  salvar() {
    ...
    // senha/par confirmação
    const pwdPairMsg = this.validatePasswordPair();
    if (pwdPairMsg) {
      // marca erro no form (genérico)
    ...
    req$.subscribe({
      next: () => {
        ...
        this.load();
        this.cancelarEdicao(); // fecha/limpa o form
        this.saving = false;
        this.submitted = false;
      },
      error: (err) => {
