...
class UserAdmin(BaseUserAdmin):
    ...
    list_display = ('id', 'username', 'email', 'first_name', 'last_name', 'type', 'is_staff', 'is_active', 'last_login', 'date_joined')
    list_filter = ('type', 'is_staff', 'is_superuser', 'is_active')
    search_fields = ('username', 'email', 'first_name', 'last_name')
    ordering = ('id',)

    # Fieldsets padrão + 'type'
    fieldsets = (
        (None, {'fields': ('username', 'password')}),
        (_('Informações Pessoais'), {'fields': ('first_name', 'last_name', 'email', 'type')}),
        (_('Permissões'), {'fields': ('is_active', 'is_staff', 'is_superuser', 'groups', 'user_permissions')}),
        (_('Datas Importantes'), {'fields': ('last_login', 'date_joined')}),
    )
    # Ao criar novo usuário via admin
    add_fieldsets = (
        (None, {
            'classes': ('wide',),
            'fields': ('username', 'email', 'first_name', 'last_name', 'type', 'password1', 'password2', 'is_staff', 'is_active'),
        }),
    )
