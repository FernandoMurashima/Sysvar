...
# =========================
# Mapa de SKU do fornecedor
# =========================
class FornecedorSkuMap(models.Model):
    Idmap = models.BigAutoField(primary_key=True)
    fornecedor = models.ForeignKey('Fornecedor', on_delete=models.CASCADE)
    cProd_vendor = models.CharField(max_length=60)                   # código interno do fornecedor (cProd)
    descricao_vendor = models.CharField(max_length=255, blank=True, null=True)

    # mapeia para SKU (revenda) ou para Produto (uso/consumo)
    produtodetalhe = models.ForeignKey('ProdutoDetalhe', on_delete=models.SET_NULL,
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(fields=['fornecedor', 'cProd_vendor'],
                                    name='uq_fornecedor_cprod')
        ]
        indexes = [
            models.Index(fields=['fornecedor', 'cProd_vendor']),
        ]

    def __str__(self):
        to = self.produtodetalhe or self.produto
        return f'{self.fornecedor.Nome_fornecedor} :: {self.cProd_vendor} → {to}'
...
class Compra(models.Model):
    ...
    nfe = models.OneToOneField('NFeEntrada', on_delete=models.PROTECT,
                               null=True, blank=True, related_name='compra')
    ...
    frete_rateado = models.DecimalField(max_digits=18, decimal_places=2,
                                        null=True, blank=True, default=0)
    desconto_rateado = models.DecimalField(max_digits=18, decimal_places=2,
                                           null=True, blank=True, default=0)
    outros_rateado = models.DecimalField(max_digits=18, decimal_places=2,
                                         null=True, blank=True, default=0)
...
class CompraItem(models.Model):
    ...
    frete_rateado_item = models.DecimalField(max_digits=18, decimal_places=2,
                                             null=True, blank=True, default=0)
    outros_rateado_item = models.DecimalField(max_digits=18, decimal_places=2,
                                              null=True, blank=True, default=0)
    ...
    class Meta:
        constraints = [
            # exatamente um dos dois FKs deve estar presente
...
class NFeItem(models.Model):
    ...
    # (espelho básico de tributos do item, expandimos depois se precisar)
    vDesc = models.DecimalField(max_digits=18, decimal_places=2, blank=True, null=True)
    vFrete = models.DecimalField(max_digits=18, decimal_places=2, blank=True, null=True)
    vOutro = models.DecimalField(max_digits=18, decimal_places=2, blank=True, null=True)
